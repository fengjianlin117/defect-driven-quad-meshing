...
GitHub Actions首次检查发现二面角约1e-14度的浮点舍入差异（例如75.00000004218944与75.00000004218946）。重建参考图的浮点量采用相对/绝对容差各1e-12；字典结构、列表顺序、整数编号和拓扑仍严格比较，类型变化也会失败。verify_snapshot与inspect_model共用这一比较逻辑。新增6项回归检查覆盖真实舍入样例及角度、拓扑、类型、长度、字段变化；方法测试仍为54项。

这项容差只用于重新计算的数据比较。全部发布文件的SHA256校验仍要求逐字节一致，冻结研究结果没有修改。

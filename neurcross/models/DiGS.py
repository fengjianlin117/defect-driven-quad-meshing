...
from .encoder import SimplePointnet

from .UNet import UNet_predict_angle
...
class Network(nn.Module):
    ...
    def forward(self, non_mnfld_pnts, mnfld_pnts=None, near_points=None, only_nonmnfld=False, latent=None):
        if self.latent_size > 0 and self.encoder is not None:
            ...
            latent_reg = None
            mods = None
        # Optimized: merge 3 decoder calls into 1 batched call
        pts_list = []
        has_mnfld = mnfld_pnts is not None and not only_nonmnfld
        has_near = near_points is not None
        splits = []
        if has_mnfld:
            pts_list.append(mnfld_pnts)
            splits.append(mnfld_pnts.shape[1])
        pts_list.append(non_mnfld_pnts)
        splits.append(non_mnfld_pnts.shape[1])
        if has_near:
            pts_list.append(near_points)
            splits.append(near_points.shape[1])
        all_pts = torch.cat(pts_list, dim=1)
        all_pred = self.decoder(all_pts, mods)
        
        idx = 0
        if has_mnfld:
            M = splits[0]
            manifold_pnts_pred = all_pred[:, idx:idx+M, :]
            idx += M
        else:
            manifold_pnts_pred = None
        N = splits[0] if not has_mnfld else splits[1]
        nonmanifold_pnts_pred = all_pred[:, idx:idx+N, :]
        idx += N
        near_points_pred = None
        if has_near:
            P = splits[-1]
            near_points_pred = all_pred[:, idx:idx+P, :]
...
class Network_predict_angle(nn.Module):
    def __init__(self, latent_size=0, in_dim=3, angle_in_dim=12, decoder_hidden_dim=256, nl='sine', decoder_n_hidden_layers=4,
                 init_type='siren', sphere_init_params=[1.6, 1.0], udf=False, vae=False):
        super().__init__()
        self.latent_size = latent_size
        self.vae = vae
        ...
        self.decoder_angle = UNet_predict_angle(C_in=angle_in_dim, C_out=1, drop_path_rate=0.1)


    def forward(self, non_mnfld_pnts, mnfld_pnts=None, near_points=None, angle_features=None, only_nonmnfld=False, latent=None):
        if self.latent_size > 0 and self.encoder is not None:
            # encoder
            q_latent_mean, q_latent_std = self.encoder(mnfld_pnts)
            q_z = dist.Normal(q_latent_mean, torch.exp(q_latent_std))
        ...
        if angle_features is not None:
            angle_pred = self.decoder_angle(angle_features, mods)
            angle_pred = angle_pred * 2.0 * torch.pi  # convert to radians
        else:
            angle_pred = None
        ...
        return {"manifold_pnts_pred": manifold_pnts_pred,
                "nonmanifold_pnts_pred": nonmanifold_pnts_pred,
                'near_points_pred': near_points_pred,
                'all_pred': all_pred,  # for merged gradient computation
                'all_pts_decoder': all_pts,  # the decoder's input (same computation graph)
                "latent_reg": latent_reg,
                }, angle_pred

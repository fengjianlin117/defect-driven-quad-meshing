...
本仓库是共同开发和论文研究的工作仓库。首版整理于 **2026-09-21**，**2026-09-26** 更新默认起始面数预算及完整流程接入，包含已实现的方法、12 个开发模型、冻结结果、查看器和论文工作稿。它尚不是已经完成未见模型验证的最终论文发布版。最新验证见[面数预算修正摘要](results/resolution_budget_20260925/README.md)。
...
这个入口复用已准备的方向场，默认先按输入规模重新生成基线，再重新诊断缺陷、选择约束并运行当前条件策略。起始目标为 `max(512, 历史基线面数, ceil(源三角面数/2))`，默认实际面数上限为 8192；可用 `--target-quads` 和 `--max-quads` 覆盖。此比例是避免过度简化的工程起点，不是最优面数或保真误差保证。完整反馈使用 `--strategy always-feedback`；历史低面数复现使用 `--resolution archived`。

基线最多进行三次后端调用，将实际面数校正到目标的 ±10% 且不超过上限；后续推荐也不能低于起始目标的 90%。未能生成合格基线时明确报错，不导出旧的低面数网格。后续几何需求和失败恢复仍可能增加面数，`--target-quads` 不是最终输出面数锁定。详见 [运行与复现说明](docs/reproduction.md)。

它**会执行后端求解**，仍需要模型对应的场、参考图和历史尺度校准材料；不是从任意 OBJ 自动完成全部前处理的通用产品入口。
